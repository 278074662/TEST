...
_response = "armalog" callExtension ("1644435bd443e&l=2cc70b2253715585235a2daa1bc919ef&sn="+str(toArray (format["ARMA2_%1_%2%3", worldName, missionName, ("_"+dayZ_serverName)])));
...
if(!_armalog)then{
	...
	diag_log "<infiSTAR.de> 644435bd443e";
	diag_log "<infiSTAR.de> 2cc70b2253715585235a2daa1bc919ef";
...
if(_armalog)then{
	...
	FN_CALL_LOG_DLL = {
		...
		_filename = format["A2_%1_%2_%3", worldName, missionName, _filename];
	...
	FN_ARMA_FETCHDATA = {
		...
		if (_url == "") exitWith {""};
		_response = "armalog" callExtension format["6%1", _url];
		if !(([_response] call IAH_fnc_getIntFromString) == 0x01) exitWith {""};
		_id = [_response,1] call fn_select_string_rest;
		_timeOut = diag_tickTime + 80;
		waitUntil {
			uiSleep 0.2;
			_response = "armalog" callExtension format["4%1", _id];
			!(([_response] call IAH_fnc_getIntFromString) == 0x02) || diag_tickTime > _timeOut
		};
		if (!(([_response] call IAH_fnc_getIntFromString) == 0x01) || (diag_tickTime > _timeOut)) exitWith {""};
		_parts = [_response, 1] call fn_select_string_rest;
		if (_parts == 1) exitWith {[_response,2] call fn_select_string_rest};
		_output = [_response,2] call fn_select_string_rest;
		for "_i" from 1 to _parts - 1 do {
			_response = "armalog" callExtension format["4%1|%2", _id, _i];
			_output = _output + ([_response,2] call fn_select_string_rest);
		};
		_output
	};
	...
	FN_ARMA_REMOTELOG = {
		...
		_url = _url call fn_clean_bad;
		if (_url == "") exitWith {""};
		"armalog" callExtension format["7rl_a2.php?%1", _url]
	};
}
else
{
...
if(isNil'dayZ_instance')then{dayZ_instance = '1';};
DAYZ_INSTANCE_LOGGING = format['   |Instance: %1 - %2%3%4 - 05-11-2017 23-03-09-v1445',dayZ_instance,productVersion select 1,productVersion select 2,productVersion select 3];
fn_executeGlobal = {
	_reobj ='HeliHEmpty' createVehicle [0,0,0];
...
_DONT_LOG = ['0'];comment 'Those IDs (player) are hidden in the Admin-Log';
...
_s_m = toArray 'example@example.com';_s_u = toArray 'UID0';_s_v = toArray "05-11-2017 23-03-09 - v1445";
_s_s = format['%1_%2_%3',worldName,missionName,dayZ_serverName];_s_s = toArray _s_s;
fn_custom_log = {
	diag_log format['<infiSTAR.de> %1: %2',_this select 0, _this select 1];
	_this call FN_CALL_LOG_DLL;
};
fn_remote_log = "
_this call fn_custom_log;
[format['owner="+str _s_m+"&owneruid="+str _s_u+"&version="+str _s_v+"&servername="+str _s_s+"&logname=%1&logentry=%2',toArray (_this select 0),toArray (_this select 1)]] call FN_ARMA_REMOTELOG;
";
fn_remote_log = compile fn_remote_log;
fnc_infiSTAR_ServerkickNew = {
	private['_clientName','_clientUID','_playerObj','_reason'];
...
iproductVersion = '05-11-2017 23-03-09-v1445';
...
if("+str _UDC+")then
{
	[] spawn {
		_log = '';
		waitUntil {_i = '24690';!isNil 'sm_done'};
		uiSleep 30;
...
[] spawn {
		...
		if(!isServer)then
		{
			...
			if(_puid != '')then
			{
				...
				if(alive player && _name == 'DEAD')then
				{
					...
					[name player,_puid,'BAN',toArray (_log)] call "+_randvar37+";
					[] spawn "+_randvar2+";
				};
	...
	_three = ""
		...
		if(hasInterface)then
		{
			...
			} forEach ['BadVarKick','hhahaaaaar','CharlieSheenkeybinds','KickOFF','yolo','runonce','notakeybind','GEFClear','GEFClose','GEFWhite','GEFRed','GEFGreen','GEFCyan','FirstHint','LoganIsTired',
	...
	while{1 == 1}do
	{
		_unit = 'HeliHEmpty' createVehicle [(random 9000)-250,(random 9000)-250,0];
		...
		uiSleep 5;
		{
			if(alive _x)then
			{
				...
				if(_puid != '')then
				{
					...
					if(_name == 'DEAD')then
					{
						[_name,_puid,_x,'BadName!'] call fnc_infiSTAR_ServerkickNew;
					}
					else
					{
						[_name,_puid,_x] call fnc_infiSTAR_PlayerLog;
					};
...
[] spawn {
	...
	_PVAH_AdminReq = {
		...
		if(_option == 1)then
		{
			...
			{
				"+_AHpos+" = _clientUID call _fn_tpKeyByUID;
		...
		if(_option == 69)then
		{
			if(_clientUID in _playableUnits)then
			{
				...
				_unit = 'HeliHEmpty' createVehicle [4000,4000,0];
	...
	while{1 == 1}do
	{
		...
		onPlayerDisconnected {
			...
			if((("+str _UDP+") || ("+str _UCL+")) && (!isNil 'DZE_CleanNull'))then{ [_uid,_name] call fnc_DisconnectCustom; };
...
"+_randvar1+" = {
		...
		[] spawn {
			...
			while{1 == 1}do
			{
				...
				
				{
					...
					'MouseButtonUp','MouseButtonClick','MouseButtonDblClick','MouseHolding','MouseZChanged',
		...
		if !("+str _UIM+")then
		{
			if("+str _UFS+")then{
				[] spawn {
					...
					_farray =
					[
						'blah.sqf','\dathrillerkiller\init.dtk',
	...
	[_name,_puid] spawn {
		...
		while{1 == 1}do
		{
			...
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'HLOG_SKICK',toArray ('CharacterID is NIL')] call "+_randvar37+";
			}
			else
			{
			...
			if !("+str _UIM+")then
			{
				...
				if(!isNil 'r_player_unconscious')then
				{
					...
					else
					{
						...
						{
							_obj setVariable['infiUnconDone','0'];comment '#12345';
						};
					};
				};
			};
	...
	if !("+str _UIM+")then
	{
		...
		diag_log format['<infiSTAR.de> 05-11-2017 23-03-09-v1445 - Successfully Loaded on Client ID12345 (%1)',time];
	}
	else
	{
		diag_log format['Successfully Loaded on Client ID12345 (%1)',time];
	};
	"+_randvar27+" = true;
...
_randvar19Local = {
	...
	{
		[] spawn "+_randvar2+";
		_log = format['dayz_playerUID [%1] is not equal to PlayerUID [%2]',dayz_playerUID,_puid];
		[_name,_puid,'BAN',toArray (_log)] call "+_randvar37+";
	};
	if!(_puid in "+str _allAdmins+")then
	{
	...
	if(!isNil '"+_randvar26+"')exitWith{
		if("+_randvar26+" != _puid)then{
			[] spawn "+_randvar2+";
			_log = format['StoredPlayerUID [%1] is not equal to PlayerUID [%2]',"+_randvar26+",_puid];
...
_toserver = {"+_randvar10+" = _this;publicVariableServer'"+_randvar10+"';};
"+_randvar37+" = _toserver;publicVariable '"+_randvar37+"';
_unit = 'HeliHEmpty' createVehicle [3500,3500,0];
_unit removeAllEventHandlers ""HandleDamage"";
...
diag_log ("<infiSTAR.de> ADDING SERVERSIDE HANDLERS");
[] spawn {
	fnc_AdminReqProceed = {
		...
		if(_option == -3)then
		{
			...
			[_object,_uid,_characterID,_class] spawn {
				...
				if(_object isKindOf "Bicycle")then
				{
                    ...
                    _object setVariable ["logistic_Cannot_Tow",false,true];
					_object setVariable ["logistic_Cannot_Lift",false,true];
				};
				clearWeaponCargoGlobal _object;
				clearMagazineCargoGlobal _object;
...
[] spawn {
	...
	fn_infiSTAR_logLock = {
		private ["_vehicle","_lock","_player"];
		...
		_log = format["%1 %2 @%3 %4 BY %5(%6)", typeOf _vehicle, _lock, mapGridPosition _vehicle, getPosATL _vehicle, _name, getPlayerUID _player];
	...
	fn_infiSTAR_local_lockUnlock = {
		private ["_vehicle","_status","_lock"];
		_vehicle = _this select 0;
		...
		if(local _vehicle)then
		{
			...
			_vehicle setVariable ["logistic_Cannot_Tow",_locktrue,true];
			_vehicle setVariable ["logistic_Cannot_Lift",_locktrue,true];
			
			if(isServer)then
			{
				_log = format["%1 %2 @%3 %4", typeOf _vehicle, _lock, mapGridPosition _vehicle, getPosATL _vehicle];
				["LockUnlockVehicle",_log] call fn_custom_log;
			}
			else
			{
