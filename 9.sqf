...
LogAdminActions = "false";	/* "LogAdminActions": Sends actions done by each admin to the server to log it to the .txt file (if infiSTAR dlls are used) and .rpt file + sends it back to all other admins. */
enableIngameLogs = "false";	/* These logs are shown in the admin menu (disabling it might lower performance cost, since it sends a lot of big strings through the network) */
...
hiddenSuperAdmin[] =
{
	"76561198127675194","76561198125361224","UID3","..."
};
...
*/
TGV = 20;	/* Terrain Grid Value - TGV: if set to 50 grass will be very low for better client FPS.. default is 12.5 ~35 is good performance and grass :) */
VDV = 1500;	/* ViewDistance Value   */
VOV = 1000;	/* ObjectViewDistance   */
SVD = 25;	/* ShadowViewDistance   */
...
blacklistedVariables[] =
{
	...
	"sni_prz_zzz_targetplayer","healit","o_fnc_arma","mlrn_exec","running_threads","catchemall5240317952","killtarget","gmtoggle","t1","fuck_me_keyp","cheatcurator"
};
